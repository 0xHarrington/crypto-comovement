...
class AutoRegressive(CryptoModel):
    """Univariate Auto-Regressive Model: latent variable model using only the past time steps"""
    ...
    def train(self, training_set):
        ...
        with warnings.catch_warnings():
            ...
            for coin, ts in self.training_data.iteritems():
                self.models[coin] = SARIMAX(ts, order = (self.lag, 0, 0), trend='c').fit(disp=False)
